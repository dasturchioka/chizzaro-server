...
async function createCourier(req, res) {
	try {
		...
		if (!fullname) {
			return res.status(400).json({ status: 'bad', msg: 'Ism familiya kiritilishi kerak' })
		}
		if (!login) {
			return res.status(400).json({ status: 'bad', msg: 'Login kiritilishi kerak' })
		}
		...
		if (login.length < 5) {
			return res
				.status(400)
				.json({ status: 'bad', msg: "Login kamida 5 ta belgidan iborat bo'lishi kerak" })
		}
		if (!password) {
			return res.status(400).json({ status: 'bad', msg: 'Parol kiritilishi kerak' })
		}
		if (password.length < 8) {
			return res
				.status(400)
				.json({ status: 'bad', msg: "Parol kamida 8 ta belgidan iborat bo'lishi kerak" })
		}
		if (!phone) {
			return res.status(400).json({ status: 'bad', msg: 'Telefon raqam kiritilishi kerak' })
		}
		...
		const newCourier = await prisma.courier.create({ data: { fullname, login, password, phone } })
...
async function getAllCouriers(req, res) {
	try {
		const couriers = await prisma.courier.findMany({ include: { orders: true } })
